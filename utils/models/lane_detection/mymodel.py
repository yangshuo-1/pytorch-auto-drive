...
class MyModel(BezierBaseNet):
    ...
    def __init__(self,
        ...
        super(MyModel, self).__init__(thresh, local_maximum_window_size)
        global_stride = 16          # 空间下采样比例 
        branch_channels = 256       # 特征通道数 

        self.backbone = MODELS.from_dict(backbone_cfg)
        self.neck = MODELS.from_dict(neck_cfg)
    ...
    def forward(self, x):
        ...
        if isinstance(x, dict) and 'out' in x.keys():
            x = x['out']

        if self.reducer is not None:
            x = self.reducer(x)
        # backbone输出是一个list，fpn输入要求tensor的list，需要转换 
        feas = []
